...
contract StrategyEurtConvex {
  ...
  constructor(address _controller, address _proxy) public {
    ...
    baseRewardPool = address(0xD814BFC091111E1417a669672144aFFAA081c3CE);
  }

  function getName() external pure returns (string memory) {
    return 'StrategyEurtConvex';
  }
  ...
  function deposit() public {
    ...
    IBooster(booster).depositAll(39, true);
  }
  ...
  function swapToEurt(uint256 maxSlippageEURT) internal {
    ...
    uint256 amountOut =
      ISwapRouter(quoter).quoteExactInput(
        abi.encodePacked(weth, uint24(3000), eurt),
        _weth
      );

    uint256 minAmountOut = amountOut.mul(10000 - maxSlippageEURT).div(10000);
  ...
  function harvest(
    ...
    if (_eurt > 0) {
      ...
      IERC20(eurt).safeApprove(curve, _eurt);

      uint256 _tokenAmount =
        ICurveFi(curve).calc_token_amount([_eurt, 0], true);
      uint256 _minimalAmount =
        _tokenAmount.mul(10000 - maxSlippageCRVAddLiquidity).div(10000);

      ICurveFi(curve).add_liquidity([_eurt, 0], _minimalAmount);
    }
