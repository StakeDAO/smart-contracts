...
const VAULT = '0x24129B935AfF071c4f0554882C0D9573F4975fEd';
const WANT = '0x075b1bb99792c9E1041bA13afEf80C91a1e70fB3';
const GOVERNANCE = '0xF930EBBd05eF8b25B1797b9b2109DDC9B0d43063';
const WANT_HOLDER = '0x282742940eE0b7ed028Bb48052Bb4922282234dA';
...
describe('ConvexSbtc', function () {
  ...
  before(async function () {
    ...
    const Strategy = await ethers.getContractFactory('StrategySbtcConvex');
  ...
  it('All funds should be in vault', async function () {
    ...
    const vaultSbtcBalance = await want.balanceOf(vault.address);
    expect(vaultBalance.eq(vaultSbtcBalance)).to.be.true;

    console.log({vaultBalance, vaultSbtcBalance});
  });

  it('should deposit 95% of vault balance in convex sBTC pool', async function () {
    this.enableTimeouts(false);
    const vaultBalance = await vault.balance();
  ...
  it('should withdraw some for the user', async function () {
    ...
    expect(balanceAfterEarn.gt(balanceBefore)).to.be.true;
    expect(expectedWithdrawFromStrategy.gte(balanceDifference)).to.be.true;
    expect(expectedUserBalanceWantAfter.eq(userWantBalanceAfter)).to.be.true;
  });
