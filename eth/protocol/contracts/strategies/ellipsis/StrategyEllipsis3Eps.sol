...
contract StrategyEllipsis3Eps {
    ...
    function deposit() public {
        ...
        if (_want > 0) {
            ...
            ILpTokenStaker(lpTokenStaker).deposit(1, _want);
        }
    }
    ...
    function _withdrawSome(uint256 _amount) internal returns (uint256) {
        ILpTokenStaker(lpTokenStaker).withdraw(1, _amount);
        return _amount;
    }
    ...
    function harvest() public {
        ...
        if (_busd > 0) {
            IERC20(busd).safeApprove(pool, 0);
            IERC20(busd).safeApprove(pool, _busd);
            ICurveFi(pool).add_liquidity([_busd, 0, 0], 0);
        }
        uint256 _want = IERC20(want).balanceOf(address(this));
        if (_want > 0) {
    ...
    function balanceOfPool() public view returns (uint256) {
        return ILpTokenStaker(lpTokenStaker).userInfo(1, address(this)).amount;
    }
